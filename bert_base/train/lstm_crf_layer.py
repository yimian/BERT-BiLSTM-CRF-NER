    ...
    def blstm_layer(self, embedding_chars):
        ...
        with tf.variable_scope('rnn_layer'):
            ...
            if self.num_layers > 1:
                cell_fw = rnn.MultiRNNCell([self._bi_dir_rnn() for _ in range(self.num_layers)], state_is_tuple=True)
                cell_bw = rnn.MultiRNNCell([self._bi_dir_rnn() for _ in  range(self.num_layers)], state_is_tuple=True)
